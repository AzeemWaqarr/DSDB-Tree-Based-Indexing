...
public:
	...
	void InsertAtTail(T d, string file, int l) {
		linkNode<T>* temp = new linkNode<T>(d, file, l);
		if (isEmpty()) {
			temp->next = NULL;
			head = temp;
		}
...
public:
	...
	Node<T>* dequeue() {	//remove from head
		if (isEmpty()) {
...
public:
	...
	{
		//bool checkIfDup = false;
		//checkIfDup = retrieve(d);
		//treeNode<T>* temp = r;
		//if (checkIfDup == true) {	//meaning data already present in tree, hence duplicate
		//	temp = searchTree(d);
		//	temp->list.InsertAtTail(d, name, l);
		//	return r;
		//}
